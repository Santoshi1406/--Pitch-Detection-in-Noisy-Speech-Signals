...
Extending 1D speech frames into a 2D multi-frame matrix, capturing the repetitive structure of voiced speech across time.
Applying multi-frame correlation to average out uncorrelated noise while reinforcing periodic pitch patterns.
Using the Radon transform on the correlation map to detect dominant periodic lines, from which the pitch period is extracted.
Post-processing with Viterbi-inspired smoothing to correct octave errors and produce a stable F0 trajectory.
...
📃 License
MIT License — see LICENSE for details.# --Pitch-Detection-in-Noisy-Speech-Signals
